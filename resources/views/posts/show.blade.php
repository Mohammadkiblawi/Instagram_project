                ...
                <div class="col-span-2 bg-white flex flex-col">
                    <div class="flex flex-row p-3 border-b border-solid border-gray-300 items-center justify-between" id="sec1">
                        <div class="flex flex-row items-center">
                            <img src="{{$post->user->profile_photo_url}}" alt="{{$post->user->username}}" srcset="" class="rounded-full h-10 w-10 me-3">
                    ...
                    <div class="border-b border-solid border-gray-300 h-full">
                        <div class="grid grid-cols-5 overflow-y-auto" id="commentArea">
                            ...
                            <div class="col-span-1 m-3">
                                <img src="{{$comment->user->profile_photo_url}}" alt="{{$comment->user->username}}" srcset="" class="rounded-full h-10 w-10">
                            </div>
                            <div class="col-span-4 mt-5 me-7">
                                ...
                                <div class="text-gray-500 text-xs">{{$comment->created_at->format('M j o')}}
                                    @can('delete',$comment)
                                    <form method="POST" action="{{route('comments.destroy', $comment->id)}}" class=inline-block>
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-xs ms-2" onclick="return confirm('Are you sure you want to delete this comment? This will delete your comment permanently.')">
